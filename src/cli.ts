...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i]
    if (arg === '-d' || arg === '--diagnostic') mode = 'diagnostic'
    else if (arg === '-H' || arg === '--human') mode = 'human'
      ...
      if (val !== 'auto' && val !== 'text' && val !== 'markdown' && val !== 'mdx' && val !== 'pandoc') {
        process.stderr.write(`promptless: --format expects one of: auto, text, markdown, mdx, pandoc\n`)
        process.exit(2)
      }
      format = val
      ...
      process.stderr.write(`promptless: unknown flag ${arg}\n`)
      process.exit(2)
    } else {
      files.push(arg)
    }
...
const HELP = `promptless — detect LLM prose tells in text files

usage:
  promptless [options] <file>...

options:
  -H, --human         Compact ANSI-highlighted view with [N] category markers (default)
  -d, --diagnostic    Elm-style per-violation output, agent-friendly
...
function resolveFormat(flag: Format, pandocFrom: string | null, file: string): ResolvedFormat {
  ...
  if (flag === 'pandoc') {
    ...
    if (!from) {
      process.stderr.write(`promptless: --format pandoc requires --from <pandoc-fmt> for ${file}\n`)
      process.exit(2)
    }
...
for (const file of args.files) {
  ...
  try {
    ...
    process.stderr.write(`promptless: could not read ${file}: ${msg}\n`)
    process.exit(2)
  }
