...
export const RULES: ViolationRule[] = [
  // ── Word Choice ──────────────────────────────────────────────────────────
  {
    ...
    severity: 'warning',
    description: 'Words like "crucial," "vital," "robust," "leverage," "delve," etc. are often LLM clichés that add noise.',
    tip: 'Warning only — not every match is a defect. Some of these words (e.g. "navigate," "foster," "harness") can be the right word in context. If the word reads as filler, delete it; otherwise leave it. Do not auto-fix — a human should decide.',
    canRemove: true,
    color: '#f59e0b',
    bgColor: 'rgba(245,158,11,0.18)',
    requiresLLM: false,
    rewriteHint: 'Consider removing overused intensifiers like "crucial", "vital", "robust", "leverage", "delve" when they read as filler — but leave contextually apt uses (e.g. "navigate" describing actual navigation) alone.',
    llmDirective: 'This is a warning, not an error. Only remove the flagged word if it is genuinely filler in context; leave it if it is doing real work (e.g. "navigate" describing actual navigation).',
  },
  {
